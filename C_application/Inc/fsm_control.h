...
typedef struct fsm
{
    ...
    num_t timestamp_last_out;   // Timestamp of when the last output was provided
    num_t time_from_last_out;   // Time elapsed from last output. Once it overcomes the time threshold, the system should compute the postprocessing
    num_t time_start_wind;      // Timestamp of the start of the current window --> used for the rerun update
    num_t time_start_last_wind; // Timestamp of the start of the last window --> used for recording the time of the last output
