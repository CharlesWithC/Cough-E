...
bigreal_t _imu_sigmoid(bigreal_t score)
{
    if (score < 0.0f) {
        bigreal_t z = expreal(score);
        return z / (1.0f + z);
    }
    return (1.0f / (1.0f + expreal(-score)));
}

bigreal_t imu_predict(bigreal_t *feats)
{
    RA_LOG_ARRAY("CLASSIFY", "imu_predict", "feats_input", feats, TOT_FEATURES_IMU_MODEL_IMU);

    bigreal_t score = 0.0f;
    int16_t current_node = 0;
    int16_t child_type = 0;
    ...
    bigreal_t res = _imu_sigmoid(score);
